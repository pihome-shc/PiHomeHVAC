#!/bin/bash
#app_name:Install Auto Hot Spot
#app_description:Install Automatic Hot Spot Capability
...
check_wificountry()
{
	...
	if [ ! -f "/etc/wpa_supplicant/wpa_supplicant.conf" ]; then
		echo "Creating a default wpa_supplicant.conf file."
        	cp "$cpath/config/wpa_supplicant.conf" /etc/wpa_supplicant
        fi
...
{
        echo "dhcpcd5 Config"
        echo "dhcpcd5 Status is " $vdhcpcd5
        if [ "$vdhcpcd5" = "N" ]; then
                echo "dhcpcd5 not installed- now installing"
                apt -q install dhcpcd5
                echo "Recheck install Status"
                check_installed
                if [ "$vdhcpcd5" = "N" ]; then
                        echo ""
                        echo ""
                        echo "dhcpcd5 failed to install. Check there is internet access"
                        echo "and try again"
                        echo "Press a key to continue"
                        read
                        menu
