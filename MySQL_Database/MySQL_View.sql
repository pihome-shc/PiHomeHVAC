...
select zone.status, zone.zone_state, zone.sync, zone.id, zone.index_id, zone.name, ztype.type, ztype.category, ts.graph_it, zs.max_c, max_operation_time, zs.hysteresis_time,
zs.sp_deadband, sid.node_id as sensors_id, ts.sensor_child_id,
ctype.`type` AS controller_type, cid.node_id as controler_id, cr.controler_child_id,
...
join controller_relays cr on zc.controller_relay_id = cr.id
join zone_type ztype on zone.type_id = ztype.id
LEFT join nodes sid on ts.sensor_id = sid.id
join nodes ctype on cr.controler_id = ctype.id
join nodes cid on cr.controler_id = cid.id
