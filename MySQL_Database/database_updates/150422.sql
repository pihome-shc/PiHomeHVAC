ALTER TABLE `schedule_daily_time_zone` ADD COLUMN IF NOT EXISTS `disabled` TINYINT(4) NOT NULL AFTER `coop`;
UPDATE `schedule_daily_time_zone` SET `disabled`= 0;
...
sdtz.status as tz_status, sdtz.zone_id, z.index_id, z.name as zone_name, zt.type, zt.category, sdtz.temperature, sdtz.holidays_id, sdtz.coop, sdtz.disabled,
