                ...
                if not sys.getsizeof(in_str) <= 25 and in_str[:1] != '0':       #here is the line where sensor are processed
                        ...
                        if len(statement) == 6 and statement[0].isdigit():      #check if received message is right format
                                ...
                                if (node_id != 0 and child_sensor_id != 255 and message_type == 1 and sub_type == 0):
                                        ...
                                        cur.execute('SELECT temperature_sensors.id, temperature_sensors.zone_id, nodes.node_id, temperature_sensors.sensor_child_id, temperature_sensors.name, temperature_sensors.graph_it FROM temperature_sensors, `nodes` WHERE (temperature_sensors.sensor_id = nodes.`id`) AND  nodes.node_id = (%s) AND temperature_sensors.sensor_child_id = (%s) AND temperature_sensors.graph_it = 1 LIMIT 1;', (node_id, child_sensor_id))
                                                ...
                                                if graph_it == 1:
                                                        ...
                                                        if zone_id == 0:
                                                                ...
                                                                if cur.rowcount > 0:
                                                                    ...
                                                                    category = int(results[zone_view_to_index['category']])
                                                                    if category != 2:
                                                                        cur.execute('INSERT INTO zone_graphs(`sync`, `purge`, `zone_id`, `name`, `type`, `category`, `node_id`,`child_id`, `sub_type`, `payload`, `datetime`) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)', (0,0,zone_id,zone_name,type,category,node_id,child_sensor_id,sub_type,payload,timestamp))
                                                                        con.commit()
